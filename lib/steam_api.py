...
async def GetPlayerSummaries(steam_id: str) -> PlayerSummary:
    async with aiohttp.ClientSession() as session:
            async with session.get(f'https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key={key}&steamids={steam_id}') as response:
...
async def ResolveVanityURL(vanityURLName: str) -> str:
     async with aiohttp.ClientSession() as session:
          async with session.get(f'https://api.steampowered.com/ISteamUser/ResolveVanityURL/v1?key={key}&vanityurl={vanityURLName}') as response:
               json = await response.json()
