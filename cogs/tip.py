...
class TipCommand(commands.Cog):
    ...
    async def tip(self, interaction: discord.Interaction, target: discord.Member, value: int) -> None:
        await interaction.response.defer(ephemeral=True)
        logger.info(f'Tip command called by {interaction.user.id} ({interaction.user.name})')
        ...
        try:
            ...
            if target.id == interaction.user.id:
                ...
                logger.info('User tried to tip themselves')
                return

            if target.bot:
                await interaction.followup.send('У бота нет кошелька! 😒', ephemeral=True)
                logger.info('User tried to tip bot')
                return
            ...
            logger.error(f'Unexpected error in tip command: {str(e)}')
            await interaction.followup.send(
...
async def setup(bot: commands.Bot):
    await bot.add_cog(TipCommand(bot), guild=discord.Object(id=settings.GUILD_ID))